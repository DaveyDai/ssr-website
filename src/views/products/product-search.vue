  ...
  export default {
    asyncData ({ store, route }) {
      return new Promise((resolve, reject) => {
        let param = {pageNo: 1, pageSize: 1000, condition: {searchName: route.query.keyword}}
        store.dispatch('postFetch', {api: 'getCategoryPro', data: param}).then(json => { // 获取分类产品
          ...
          store.commit('setSearchProduct', {records: [], keyword: route.query.keyword})
          reject(error)
        })
      })
    },
    ...
    methods: {
      ...
      searchQuery () {
        let param = {pageNo: 1, pageSize: 1000, condition: {searchName: route.query.keyword}}
        this.$store.dispatch('postFetch', {api: 'getCategoryPro', data: param}).then(json => { // 获取分类产品
          console.log('搜索结果：', json)
          json.keyword = route.query.keyword
          this.$store.commit('setSearchProduct', json)
          resolve()
        }).catch(error => {
          this.$store.commit('setSearchProduct', {records: [], keyword: route.query.keyword})
          reject(error)
        })
      },
