<template>
  <div class="create-account">
    ...
    <!-- 邮箱地址 -->
    <vava-input v-if="!verificationShow" maxlength="200" v-model="sendCodeParam.sendToEmail" class="create-account-input" placeholder="Email Address"></vava-input>
    <!-- 邮箱验证码输入框 -->
    ...
    methods: {
      ...
      loginCheckData (loginParam) {
        this.$store.dispatch('postFetch', {api: 'signIn', data: loginParam}).then(data => {
          ...
          this.$utils.message('Register Success.')
          // this.$utils.message(error && error.message || 'The system is busy. Please try to refresh it.')
          this.$bar.finish()
        })
      },
